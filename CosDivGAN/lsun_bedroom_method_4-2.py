...
def train_gan(epoch_setting):
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            diversity_loss = weight * (diversity_loss_d_fake_mid + diversity_loss_d_real_mid)

            # 내쉬 균형 편차에 따른 D와 G에 대한 다양성 패널티 가중치 결정
            # 균형에 가까울수록 D에 적용, 균형에서 멀수록 G에 적용
            d_factor = 1.0 - nash_deviation  # 균형에 가까울수록 1에 가까움
            g_factor = nash_deviation  # 균형에서 멀수록 1에 가까움

            # 패널티 적용
            penalty_d = diversity_lambda * d_factor * diversity_loss
            penalty_g = diversity_lambda * g_factor * diversity_loss

            errD = errD_real + errD_fake + penalty_d
            # D를 업데이트 합니다
            optimizerD.step()
            ...
            if i % 50 == 0:
                print(
                    '[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tD(x): %.4f\tD(G(z)): %.4f / %.4f\tDiv: %.4f'
...
def main():
    # 여러 에폭 설정으로 학습 실행
    epoch_settings = [15,30,45,60,75,90,115]

...
def save_final_model(netG, netD, fid_score=None, epoch_setting=None):
    ...
    # 에폭 설정에 따른 모델 경로
    model_path = os.path.join(save_dir, f"final_model_epoch_{epoch_setting}.pth")
...
def train_gan(epoch_setting):
    ...
    lr_D = 0.0001  # 판별자 학습률 (더 높게 설정)
    lr_G = 0.0004  # 생성자 학습률 (더 낮게 설정)
    ...
    diversity_lambda = 0.5  # 초기값 더 낮게 설정

    # 적응적 가중치 조정을 위한 설정 (점진적으로 증가하도록 수정)
    adaptive_weight = True
    diversity_max = 10.0  # 최대 다양성 가중치
    diversity_step = 0.5  # 매 에폭마다 증가시킬 양 (더 점진적으로)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if i % 50 == 0:
                print(
                    '[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tD(x): %.4f\tD(G(z)): %.4f / %.4f\tDiv: %.4f\tDiv_λ: %.2f'
                    % (epoch, num_epochs, i, len(dataloader),
                       errD.item(), errG.item(), D_x, D_G_z1, D_G_z2,
                       div_score, diversity_lambda))
    ...
    real_dir = f"../DCGAN_cosine_epoch_{epoch_setting}_real_images"
    fake_dir = f"../DCGAN_cosine_epoch_{epoch_setting}_fake_images"
...
def main():
    # 여러 에폭 설정으로 학습 실행
    epoch_settings = [15, 30, 45,60,75,90]
    ...
    print(f"\n최적의 에폭 설정: {best_epoch} (FID 점수: {results[best_epoch]:.2f})")

    # 결과 시각화
    plt.figure(figsize=(10, 6))
    epochs = list(results.keys())
    scores = [results[e] for e in epochs]

    plt.plot(epochs, scores, 'o-', color='blue', linewidth=2, markersize=8)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.title('FID Score vs Training Epochs', fontsize=15)
    plt.xlabel('Number of Training Epochs', fontsize=12)
    plt.ylabel('FID Score (Lower is Better)', fontsize=12)

    # 최적의 에폭 표시
    plt.scatter([best_epoch], [results[best_epoch]], color='red', s=100, zorder=5)
    plt.annotate(f'Best: {results[best_epoch]:.2f}',
                 xy=(best_epoch, results[best_epoch]),
                 xytext=(best_epoch + 2, results[best_epoch] + 2),
                 arrowprops=dict(facecolor='black', shrink=0.05, width=1.5),
                 fontsize=10)

    plt.tight_layout()
    plt.savefig('epoch_comparison_results.png', dpi=300)
    '''

...
def train_gan(epoch_setting):
    ...
    # Adaptive Balancing을 위한 변수 초기화
    ema_d_loss = 1.0  # D 손실 초기값
    ema_g_loss = 1.0  # G 손실 초기값
    ema_alpha = 0.9  # 지수 이동 평균(EMA) 가중치 (α)
    original_diversity_lambda = diversity_lambda  # 초기 다양성 가중치 저장 (λ₀)
    beta = 1.0  # 균형 편차 민감도 (β)
    bound_factor = 5.0  # 가중치 제한 계수 (c)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            # 초기화
            effective_lambda = original_diversity_lambda  # 기본값으로 초기화

            # 현재 손실 통계에 기반한 다양성 가중치 계산
            if i > 0 or epoch > 0:  # 첫 번째 배치는 기본값 사용
                # 손실 비율 계산
                loss_ratio = ema_d_loss / (ema_g_loss + 1e-8)

                # 대수적 손실 비율 계산
                import math
                delta = math.log(loss_ratio + 1e-8)
                ...
                max_lambda = original_diversity_lambda * bound_factor
            ...
            # 지수 이동 평균(EMA) 업데이트
            ema_d_loss = ema_alpha * ema_d_loss + (1 - ema_alpha) * current_d_loss
            ema_g_loss = ema_alpha * ema_g_loss + (1 - ema_alpha) * current_g_loss

            # 다양성 메트릭 계산 및 추적 (모니터링용)
            if i % 50 == 0:
                with torch.no_grad():
                ...
                # 확장된 훈련 상태 출력 (균형 비율과 람다 값 포함)
                print(
                    '[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tD(x): %.4f\tD(G(z)): %.4f / %.4f\tDiv: %.4f\tRatio: %.2f\tλ: %.2f'
                    % (epoch, num_epochs, i, len(dataloader),
                       errD.item(), errG.item(), D_x, D_G_z1, D_G_z2,
                       div_score, ema_d_loss / (ema_g_loss + 1e-8), effective_lambda))

        # 에폭 종료 후 학습률 스케줄러 업데이트
        schedulerD.step()
        schedulerG.step()

    # 시각화 부분에 람다 값 추이 그래프 추가

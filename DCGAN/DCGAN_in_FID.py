...
def main():
    ...
    # clean-FID 측정
    # 저장할 디렉토리 설정
    real_dir = "../DCGAN_real_images"
    fake_dir = "../DCGAN_fake_images"
    os.makedirs(real_dir, exist_ok=True)
    os.makedirs(fake_dir, exist_ok=True)

    # 1️⃣ 50,000장 샘플링하여 저장
    num_samples = 50000
    real_samples = []
    fake_samples = []

    # 1️⃣ 배치 단위로 이미지 저장 (전체 배치 반복)
    num_full_batches = num_samples // batch_size  # 390개 배치
    remaining_samples = num_samples % batch_size  # 80개 남음

    for i in range(num_full_batches):
        real_samples.append(next(iter(dataloader))[0].to(device))
        fake_samples.append(netG(torch.randn(batch_size, nz, 1, 1, device=device)).detach())

    # 2️⃣ 남은 80장 추가 처리 (마지막 배치)
    if remaining_samples > 0:
        real_samples.append(next(iter(dataloader))[0][:remaining_samples].to(device))
        fake_samples.append(netG(torch.randn(remaining_samples, nz, 1, 1, device=device)).detach())
